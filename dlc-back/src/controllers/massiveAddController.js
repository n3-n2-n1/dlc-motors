...
const massiveAdd = async (req, res) => {
    try {
        ...
        for (const row of sheetData) {
            await db.query(
                ...
                [row.Codigo, row.Producto, row.Rubro, row.CodBarras, row.Precio, row.Stock, row.Image, row.hasStock, row.Origen, row.CodTango, row.CodOEM, row.marcasCompatibles, row.Devoluciones, row.Tag, row.Kit]
            );
        }

        res.status(200).send({ message: 'Datos insertados correctamente en la base de datos.' });
    } catch (error) {
